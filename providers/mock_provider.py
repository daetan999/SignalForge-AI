...
def _extract_requirements(
    ...
    user_evidence = _find_evidence(
        documents, lambda line: bool(re.search(r"\b[\d,]+\s+(?:intended\s+)?users\b", line))
    )
    if users is not None and user_evidence:
        requirements.append(
            _req("users", f"Support approximately {users:,} intended users.", *user_evidence)
        )

    patterns: list[tuple[str, str, tuple[str, ...]]] = [
        (
            "data",
            "Use the documented enterprise data sources.",
            ("bigquery", "google drive", "sharepoint", "data source"),
        ),
        (
            "integration",
            "Integrate with the documented existing environment.",
            ("existing cloud", "integration", "identity provider"),
        ),
        (
            "security",
            "Enforce the documented identity and audit controls.",
            ("role-based", "rbac", "audit logging", "least privilege"),
        ),
    ]
    for category, summary, keywords in patterns:
        evidence = _find_evidence(
            documents, lambda line, keys=keywords: any(key in line for key in keys)
        )
        if evidence:
    ...
    if latency_evidence:
        requirements.append(
            _req("performance", "Meet the documented response-latency target.", *latency_evidence)
        )
    ...
    if scale_evidence:
        requirements.append(
            _req(
                "performance",
                "Support the documented workload scale and request volume.",
                *scale_evidence,
            )
        )
    ...
    if timeline is not None and timeline_evidence:
        requirements.append(
            _req("timeline", f"Deliver the pilot within {timeline} weeks.", *timeline_evidence)
        )
    ...
    approved_budget = _find_evidence(
        ...
        lambda line: (
            "budget" in line
            and any(word in line for word in ("approved", "estimated", "allocated"))
            and "not" not in line
        ),
    )
    if approved_budget:
        requirements.append(
            _req("commercial", "Operate within the documented pilot budget.", *approved_budget)
        )
    return requirements
...
def _unknown_requirements(
    ...
    if budget_status == "unknown":
        unknowns.append(
            Requirement(
                category="commercial",
                requirement="Confirm an approved pilot budget.",
                status="unknown",
            )
        )
    if data_sensitivity == "unknown":
        unknowns.append(
            Requirement(
                category="security",
                requirement="Confirm the formal data classification and PII scope.",
                status="unknown",
            )
        )
    if not any(
        item.category == "performance" and "scale" in item.requirement.lower()
        for item in requirements
    ):
        unknowns.append(
            Requirement(
                category="performance",
                requirement="Confirm peak concurrency and request volume.",
                status="unknown",
            )
        )
    return unknowns
...
def _data_sensitivity(text: str) -> str:
    unknown_pattern = (
        r"(?:not confirmed|unknown|tbd)[^\n.]*(?:pii|data classification)"
        r"|(?:pii|data classification)[^\n.]*(?:not confirmed|unknown|tbd)"
    )
    if re.search(
        unknown_pattern,
        text,
